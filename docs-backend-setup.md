...
- `SUPABASE_SERVICE_ROLE_KEY`: Supabase の Service Role Key (オプション。認証なしで全件取得等を行う場合に使用)

※ 現在の `netlify/functions/_lib/config.js` では `SUPABASE_SERVICE_ROLE_KEY` が優先されます。
...
- `SELECT`: `true` (Netlify Function 経由で取得するため。クライアントから直接参照されないよう Anon Key ではなく Service Role Key を推奨)
